...
class UpdatePostulanteRequest extends FormRequest
{
    ...
    {
        // The ID is passed as 'postulante' in the route parameters
        $ci = $this->route('postulante');

        return [
            'ci' => ['required', 'numeric', \Illuminate\Validation\Rule::unique('usuarios', 'ci')->ignore($ci, 'ci')],
            ...
            'correo_electronico' => ['required', 'email', \Illuminate\Validation\Rule::unique('usuarios', 'correo_electronico')->ignore($ci, 'ci')],
